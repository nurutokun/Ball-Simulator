...
public class Client {
	...
	public Client(MasterRender masterRender) {
		...
		viewport = new Viewport(masterRender, world);
	...
	public void update(long timePassed) {
		...
		if(!paused) {
			...
			if(updateGameLogic) {
				...
				player.update(timePassed, Mouse.getX(true) + (int) viewport.getCamera().getX(),
						Mouse.getY(true) + (int) viewport.getCamera().getY());
				
			}
			
		}
		
//		viewport.update(timePassed, player.getX(), player.getY());
		viewport.update(timePassed, 0d, 0d);
	...
	private void handleKeyboardInput() {
		...
		if(togglePause) {
			...
			setPaused(showPauseScreen() || inventory.isActive());
...
//		viewport.setCameraLocked(!MouseInput.isClamped());
	...
	public void render(Graphics g, int width, int height) {
		...
		viewport.render(g, width, height);
